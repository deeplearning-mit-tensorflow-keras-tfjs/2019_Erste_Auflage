...
# und speichern des trainierten Modells mit tf.saved_model.builder.SavedModelBuilder()
# 
...
# Aufruf des Modells
fashion_model_out = fashion_model(w,b,X)
...
# 
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=fashion_model_out,labels=Y))
...
with tf.Session() as sess:
    ...
    builder = tf.saved_model.builder.SavedModelBuilder("./model_export")
    for i in epochs:
        sess.run(train_op, feed_dict={X: train_data, Y: train_labels})
        loss = sess.run(cost,feed_dict={X: train_data, Y: train_labels})
        accuracy = np.mean(np.argmax(sess.run(fashion_model_out,feed_dict={X: train_data, Y: train_labels}),axis=1) == np.argmax(train_labels,axis=1))
        
        if(i%10==0):
            print("Epoch {} // Accuracy: {} Loss:{}".format(i,accuracy,loss))

    # Ausgabe der Vorhersage für das erste Element von eval_data
    # predictions = sess.run(fashion_model_out,feed_dict={X:[eval_data[0]]})
    ...
    builder.add_meta_graph_and_variables(sess, ["train"],
        ...
        inputs= {"X": X},
        outputs= {"finalnode": fashion_model_out})
        })
    builder.save()
